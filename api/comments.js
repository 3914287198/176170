...
// 引入钉钉通知功能
const { sendNewCommentNotice } = require('./dingtalk-notice');
// 引入地理位置获取功能
...
async function initializeDatabase() {
  try {
    ...
    try {
      ...
      try {
        await sql`ALTER TABLE comments ADD COLUMN location TEXT`;
      } catch (e) {
        // 列可能已经存在，忽略错误
      }
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
      ...
      try {
        ...
        if (isAdmin && adminToken) {
          ...
          const comments = commentsResult.map(row => ({
            ...
            ip: row.ip || '未知', // 管理员访问时不进行IP地址隐私保护处理
    ...
    try {
      ...
      // 插入新评论到数据库
      await sql`
        INSERT INTO comments (id, name, content, date, approved, ip, location)
        VALUES (${newComment.id}, ${newComment.name}, ${newComment.content}, ${newComment.date}, ${newComment.approved}, ${newComment.ip}, ${location})
      `;
      ...
      try {
        ...
        await sendNewCommentNotice(name, content, beijingTime, newComment.id, location);
      } catch (error) {
        console.error('发送钉钉通知失败:', error.message);
      }
      
      return res.status(201).json({...newComment, location});
    ...
    try {
      ...
      if (reply !== undefined) {
        ...
        // 验证更新后的评论
        const updatedComment = await sql`SELECT ip, location FROM comments WHERE id = ${id}`;
      ...
      const updatedComment = {
        ...
        location: updatedResult[0].location || '',
